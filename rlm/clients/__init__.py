...
from rlm.clients.ollama_client import OllamaClient  # Register the Ollama client
...
def get_client(
    ...
    if backend == "openai":
        ...
        return OllamaClient(**backend_kwargs)
    else:
        raise ValueError(
            f"Unknown backend: {backend}. Supported backends: ['openai', 'vllm', 'portkey', 'openrouter', 'litellm', 'anthropic', 'azure_openai', 'gemini', 'vercel', 'ollama']"
        )
